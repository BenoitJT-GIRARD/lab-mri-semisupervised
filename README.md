...
100 labelled brain MRIs, 1 406 unlabelled ones, and a protocol built to give a negative
answer if that is the answer. It gives one.

![Label efficiency: the task, and then the question](reports/figures/label_efficiency.png)

**Project status** — finished, and archived in a runnable state. The CI does not run on a
schedule. Every number below comes from a run whose manifest is in `reports/experiments/`,
naming its dataset fingerprint, seeds, package versions, GPU and git revision.

## The problem

Labelling a brain MRI takes a radiologist. Unlabelled scans are comparatively free. So the
standing question in medical imaging is whether the free ones can be made to pay: cluster
the unlabelled pool, turn the clusters into pseudo-labels, pre-train on those, then
fine-tune on the few real labels you have.

The question is easy to ask and hard to answer honestly, because the experiment that tests
it has three ways of answering yes when it should say no. The pseudo-labels can be built
using the test labels. The evaluation images can appear in the pool. And the
semi-supervised arm can simply take more gradient steps than the arm it is compared to —
measuring budget, not method.

Each of those makes a real effect out of nothing, and none of them looks unusual in the
code. So the object of this repository is not the model. **It is a protocol that can say
no** — and the evidence that it would have said yes had there been anything to say yes to.

## What it does

Eight arms, sharing folds, architecture and starting weights, on 25 folds — five stratified
folds, five repeats — over 99 distinct evaluation images with 59 training labels each.

Five design rules make the comparison mean something:

- **Identity from content.** SHA-256 of the decoded pixels, not the path. The duplicate
  rules are written down because they are decisions: a leaked copy leaves the *pool* and
  never the evaluation set, a repeated evaluation image is reduced to one, and the same
  content under two labels stops the build rather than being quietly resolved.
...
- **A control for every arm.** Each pre-training arm has a twin that sees the same images
  ...
  reason a null result here means anything.
...
![Paired differences across shared folds, with bootstrap intervals](reports/figures/paired_differences.png)

**Every arm that touches the unlabelled pool sits below the baseline that ignores it.** Not
significantly — every interval crosses zero — but consistently, at every budget, under
every variant.

### The pseudo-labels do carry information

![Each arm against its own control](reports/figures/mechanisms.png)
...
something real, and something a shuffle destroys. Self-training does not: its two intervals
span zero, so its pseudo-labels add nothing its own control does not already have.
...
the shuffle does, not how much the real labels add. Read against the control it looks like a
win; read against the baseline the same arm is behind.
...
histogram equalisation in front of it improves the supervised baseline by **+0.013 [+0.003,
+0.024], p = 0.006** — the only comparison in this project that clears conventional
significance — and collapses the clustering's agreement with the labels from an ARI of
**0.457 ± 0.096** to **0.205 ± 0.179**. Remove the global intensity differences and the
structure the clustering was finding goes with them. On this dataset contrast happens to
correlate with the class; it is not what a radiologist would be looking at.
...
per hundred labels** between 10 and 20, and **0.03** between 20 and 40. Still positive at 59
— labelling more would still pay — but flattening.
...
`build_figures.py` also draws the twelve most-missed scans with their scores, which is
the figure that actually shows what "harder" means here. It is the one output this
repository does not ship: it contains the scans themselves, and they are not ours to
redistribute. Run the script and it appears in `reports/figures/`.

**A sensitivity-first operating point does not transport.** Fixing a 90 % recall floor on
ten validation positives yields **82.0 %** realised recall on the test fold, and misses the
floor on 51 folds out of 100. It also sits above the F1 threshold on 95 folds out of 100 —
trading 7 points of recall for 2 of precision. On this dataset the F1 threshold is the more
sensitive of the two, which is the reverse of what a screening frame wants. Both are
...
Because the same machinery produced a positive answer before, and the difference between
the two is priced rather than asserted. Three leaks, all pointing the same way, plus a
confound:
...
labelled images out of the pool never fired. 31 of the 99 distinct evaluation images were in
the pool the semi-supervised arm pre-trained on.
...
That the errors concentrate on the duplicated images makes the leak worse than it first
looks: what leaked was not a random sample, it was precisely the cases the model gets wrong.
...
├── reports/figures/         # every figure build_figures.py draws, including the ROC curves
│                          #   and the arm comparison this README does not reproduce
...
is in that script. It is built that way for three reasons: the cell text is versioned and
diffable instead of being buried in JSON, the imports cannot drift away from the
`mri_semisupervised` package because they are written against it, and `nbconvert --execute`
replays them with no manual step — so a stored output is always a capture of a real run.
...
**The MRI images are not redistributed here, and this repository does not hold the right to
redistribute them.** They are third-party medical images obtained as a fixed archive; their
terms are not public, so anyone reproducing this supplies their own copy and points
`MRI_DATA_DIR` at it.
...
That rule cost one figure. `reports/figures/hardest_images.png` renders twelve of the
scans, and it had been committed with the plots — a claim about redistribution is worth no
more than the file listing that contradicts it. It is now generated on demand and ignored
by git; every other figure in `reports/figures/` plots derived numbers only.
